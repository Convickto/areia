...
export class PieceManager {
    ...
    hardDrop() {
        ...
        while (!this.checkCollision({ ...this.currentPiece, y: this.currentPiece.y + this.config.tileSize })) {
            this.currentPiece.y += this.config.tileSize;
        }
    }

    checkCollision(testPiece) {
        // Implementação básica - será expandida no CollisionSystem
        return false;
    }
    ...
    renderPiece(ctx, piece) {
        ...
        for (const block of piece.blocks) {
            ...
            ctx.beginPath();
            ctx.roundRect(x, y, this.config.tileSize, this.config.tileSize, 5);
            ctx.fill();
            ctx.stroke();
            ...
            ctx.beginPath();
            ctx.roundRect(x + 2, y + 2, this.config.tileSize - 4, this.config.tileSize - 4, 3);
            ctx.fill();
        }
    }
    ...
    renderNextPiece(ctx) {
        ...
        for (const block of this.nextPiece.blocks) {
            ...
            ctx.beginPath();
            ctx.roundRect(x, y, previewSize, previewSize, 3);
            ctx.fill();
        }
    }
