...
export class PieceManager {
    ...
    generateNextPiece() {
        ...
        this.nextPiece = {
            type: type,
            color: color,
            blocks: this.pieces[type].map(function(block) {
                return { 
                    x: block[0] * this.config.tileSize, 
                    y: block[1] * this.config.tileSize 
                };
            }.bind(this)),
            rotation: 0
        };
    ...
    rotate() {
        ...
        const originalBlocks = this.currentPiece.blocks.slice();
        const centerBlock = this.currentPiece.blocks[1] || { x: 0, y: 0 };

        // Rotação 90 graus no sentido horário
        const rotatedBlocks = originalBlocks.map(function(block) {
    ...
    hardDrop() {
        if (!this.currentPiece) return;
        this.currentPiece.y += this.config.tileSize * 10;
    }
    ...
    renderNextPiece(ctx) {
        ...
        for (let i = 0; i < this.nextPiece.blocks.length; i++) {
            const block = this.nextPiece.blocks[i];
