...
export class UIManager {
    ...
    async initialize() {
        try {
            ...
            // Continua sem UI se houver erro
            this.isInitialized = false;
        }
    }
    ...
    createUI() {
        ...
        const uiHTML = `
            <div style="position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.8); padding: 10px; border-radius: 5px; color: #00e6e6;">
            ...
            <div style="position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); display: flex; gap: 10px;">
                <button id="pause-btn" style="background: #00e6e6; color: #000; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">Pausar</button>
                <button id="restart-btn" style="background: #e600e6; color: #fff; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer;">Reiniciar</button>
            </div>
        `;
